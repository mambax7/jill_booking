    ...
    if (!empty($jbi_sn)) {
        ...
        foreach ($timeArr as $t => $time) {
            for ($w = 0; $w < 7; $w++) {
                ...
                $weektime[$t][$w]['jbt_week'] = (strpos($jbt_week, (string)$w) !== false) ? "<input type='checkbox' name='jb_week[$jbt_sn][]'  value='$w' >" : "<span style='color:#D44950'><i class='fa fa-times'></i></span>";
            }
        }
...
/**
 * @param string $jb_start_date
 * @param string $jb_end_date
 * @param string $str
 * @return mixed
 */
